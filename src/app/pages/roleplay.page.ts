              ...
              @for (session of sessions(); track session.id) {
                <div class="bg-white rounded-xl border border-secondary-200 shadow-sm p-5 hover:border-primary-300 transition-colors group cursor-pointer" (click)="loadSession(session.id)">
                  <div class="flex items-start justify-between gap-4">
                    ...
                    <button type="button" (click)="confirmDelete(session); $event.stopPropagation()" class="p-2 rounded-lg border border-red-100 hover:bg-red-50 transition-colors text-red-500 focus-visible:ring-2 focus-visible:ring-red-500 outline-none" title="Delete Session" aria-label="Delete Session">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
