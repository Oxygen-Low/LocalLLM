...
describe('Rate limiting', () => {
  ...
  it('auth limiter sets correct limit value in headers', async () => {
    ...
    // RateLimit-Limit reflects the authLimiter max (1000 in test env, 10 otherwise)
    const limit = res.headers['ratelimit-limit'];
    const expectedLimit = process.env.NODE_ENV === 'test' ? '1000' : '10';
    assert.equal(limit, expectedLimit, `Expected auth rate limit of ${expectedLimit}, got ${limit}`);
  });
...
describe('Docker/Container endpoints', () => {
  ...
  it('POST /api/coding-agent/containers accepts GitLab HTTPS URL without GitHub token', async () => {
    ...
    // 500 is accepted due to "Docker create error" in sandbox environments.
