...
describe('Rate limiting', () => {
  ...
  it('auth limiter sets correct limit value in headers', async () => {
    ...
    // RateLimit-Limit reflects the authLimiter max (1000 in test mode, 10 otherwise)
    const limit = res.headers['ratelimit-limit'];
    assert.equal(limit, '1000', `Expected auth rate limit of 1000, got ${limit}`);
  });
...
describe('Docker/Container endpoints', () => {
  ...
  it('POST /api/coding-agent/containers accepts GitLab HTTPS URL without GitHub token', async () => {
    ...
    // (500 is possible if runCommandAsync throws due to container execution failing without docker daemon running properly locally)
